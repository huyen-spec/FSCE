...
import torch.nn.functional as F
from fsdet.conv.gmm_conv import GMMConv
...
class StandardROIHeads(ROIHeads):
    ...
    def _init_box_head(self, cfg):
        ...
        # pdb.set_trace()
        new_boxhead_output = 1280
        self.box_predictor_new = ROI_HEADS_OUTPUT_REGISTRY.get(self.output_layer_name)(
            cfg, new_boxhead_output, self.num_classes, self.cls_agnostic_bbox_reg
        )

        fc_channels =   1024   #boxes_feature.shape[1]
        self.relation_fc_1 = nn.Linear(fc_channels, 256)
        self.relation_fc_2 = nn.Linear(256, 256)

        self.gaussian = GMMConv(fc_channels, fc_channels, dim=2, kernel_size = 25).to('cuda')
        self.sg_conv_1 = nn.Linear(fc_channels, 512).to('cuda')
        self.sg_conv_2 = nn.Linear(512, 256).to('cuda')
    ...
    def forward(self, images, features, proposals, targets=None):
        ...
        if self.training:
            ...
            combine, losses = self._forward_box(features_list, proposals)  # get losses from fast_rcnn.py::FastRCNNOutputs
            # return proposals, losses  # return to rcnn.py line 201

            # boxes_pred = combine[2]
            # boxes_prob = combine[3]
            # boxes_feature = combine[1]
            new_losses = self._forward_box_next(combine, proposals)
            return proposals, losses, new_losses
        else:
            # pred_instances = self._forward_box(features_list, proposals)
            pred_instances = self._forward_box_next(features_list, proposals)
            return pred_instances, {}
    ...
    def _forward_box(self, features, proposals):
        ...
        box_pred = outputs.predict_boxes()
        box_prob = outputs.predict_probs()

        # pdb.set_trace()
        if self.training:
            return [self.box_predictor.cls_score, box_features, box_pred, box_prob], outputs.losses()
        else:
            pred_instances, _ = outputs.inference(
                self.test_score_thresh, self.test_nms_thresh, self.test_detections_per_img
            )
    ...
    def _forward_box_next(self, combine, proposals):
        ...
        def gcn(boxes_pred, boxes_prob, boxes_feature, cls_score):
            # import torch.nn as nn
            # import torch.nn.functional as F
            # from fsdet.conv.gmm_conv import GMMConv
            # fc_channels = boxes_feature.shape[1]
            # relation_fc_1 = nn.Linear(fc_channels, 256)
            # relation_fc_2 = nn.Linear(256, 256)

            # gaussian = GMMConv(fc_channels, fc_channels, dim=2, kernel_size = 25).to('cuda')
            # sg_conv_1 = nn.Linear(fc_channels, 512).to('cuda')
            # sg_conv_2 = nn.Linear(512, 256).to('cuda')

            assert len(boxes_pred) == 1
            assert boxes_feature.shape[0] == 128
            num_rois = boxes_pred[0].shape[0]

            
            # adjacency matrix
            z = self.relation_fc_1.cuda()(boxes_feature)
            z = F.relu(self.relation_fc_2.cuda()(z))
            ...
            # input matrix X
            cls_w = cls_score.weight
            cls_prob = boxes_prob[0]
            represent = torch.mm(cls_prob, cls_w).to('cuda')
            ...
            # num_classes = 262
            bbox_pred_reshape = boxes_pred[0].view(-1, 262 , 4)
            bbox_pred_cls = torch.zeros(num_rois, 4)
            remove_list = []
            for i, cls in enumerate(cls_pred):
                if cls == 0:
                    remove_list.append(i)  # -> bbox_pred_cls remove elements in remove_list
                bbox_pred_cls[i] = bbox_pred_reshape[i][cls-1]            

            bbox_pred_ctr = bbox_pred_cls[:, 0:2] + bbox_pred_cls[:, 2:4]

            pdb.set_trace()

            relation = torch.empty(2, 32 * num_rois, dtype=torch.long).to('cuda')
            ...
            U = torch.stack([d, theta], dim=1).to('cuda')
            ...
            new_f = torch.cat([boxes_feature, h], dim=1)
        ...
        new_f = gcn(boxes_pred, boxes_prob, boxes_feature, cls_score)
        if self.training:
            new_losses = self.calculate_added_loss(new_f, proposals)

            return new_losses
        else:
            pred_instances = self.calculate_added_loss(new_f, proposals)
            return pred_instances


    def calculate_added_loss(self, new_box_features, proposals):
        pred_class_logits, pred_proposal_deltas = self.box_predictor_new(new_box_features)
...
@ROI_HEADS_REGISTRY.register()
class StandardROIHeads_Copy(ROIHeads):
    """
    It's "standard" in a sense that there is no ROI transform sharing
    or feature sharing between tasks.
    The cropped rois go to separate branches directly.
    ...
    def __init__(self, cfg, input_shape):
        super(StandardROIHeads_Copy, self).__init__(cfg, input_shape)
        self._init_box_head(cfg)
    ...
    def _forward_box_next(self, combine, proposals):
        ...
        pdb.set_trace()
        boxes_pred = combine[2]    #    B x 128 x 1024(262*4)

        boxes_prob = combine[3]      # B x 128 x 263 (262 classes + background)
        boxes_feature = combine[1]   # B x 128 x 1024 (vector of each roi)
        cls_score = combine[0]      # 263 x 1024

        B = len(combine[2])
        boxes_feature = boxes_feature.reshape(B,128,1024)
        ...
        def gcn(boxes_pred, boxes_prob, boxes_feature, cls_score):
            ...
            # assert len(boxes_pred) == 1
            assert boxes_feature.shape[1] == 128
            num_rois = boxes_pred[0].shape[0]
            ...
            # pdb.set_trace()
            tt = z.transpose(2,1)
            eps = torch.matmul(z, tt)
            _, indices = torch.topk(eps, k=32, dim=1)
            # pdb.set_trace()

            # input matrix X
            cls_w = cls_score.weight   #  263 1024
            cls_prob = torch.stack(boxes_prob)   # B 128 263
            represent = torch.matmul(cls_prob, cls_w).to('cuda')   # B 128 1024
            cls_pred = torch.max(cls_prob, 2)[1]   # B x 128
            ...
            # 
            # num_classes = 262
            bbox_pred_reshape = torch.stack(boxes_pred)
            bbox_pred_reshape = bbox_pred_reshape.view(-1, 128, 262 , 4)  # B 128 262 4
            bbox_pred_cls = torch.zeros(bbox_pred_reshape.shape[0], num_rois, 4)   # B 128 4
            # remove_list = []
            # for i, cls in enumerate(cls_pred):

            bbox_pred_reshape2 = bbox_pred_reshape.view(-1, 262 , 4)
            bbox_pred_cls2 = bbox_pred_cls.view(-1, 4)
            pdb.set_trace()
            cls_pred2 = cls_pred.view(-1)
            for i in range(cls_pred2.shape[0]):   # i: 0->  Bx128 
                cls = cls_pred2[i]
                # if cls == 0:
                #     remove_list.append(i)  # -> bbox_pred_cls remove elements in remove_list
                # pdb.set_trace()
                bbox_pred_cls2[i] = bbox_pred_reshape2[i][cls-1]            

            bbox_pred_cls = bbox_pred_cls2.reshape(bbox_pred_reshape.shape[0], num_rois, 4)
            pdb.set_trace()
            bbox_pred_ctr = bbox_pred_cls[..., 0:2] + bbox_pred_cls[..., 2:4]

            pdb.set_trace()

            relation = torch.empty(bbox_pred_reshape.shape[0], 2, 32 * num_rois, dtype=torch.long).to('cuda')

            pdb.set_trace()

            relation[:,0] = torch.Tensor(list(range(num_rois)) * 32).repeat(bbox_pred_reshape.shape[0],1)  # , type=torch.long)
            relation[:,1] = indices.view(indices.shape[0], -1)
            ...
            d = torch.sqrt((coord_i[..., 0] - coord_j[..., 0]) ** 2 + (coord_i[..., 1] - coord_j[..., 1]) ** 2)
            theta = torch.atan2((coord_j[..., 1] - coord_i[..., 1]), (coord_j[..., 0] - coord_i[..., 0]))
            U = torch.stack([d, theta], dim=2).to('cuda')
...
@ROI_HEADS_REGISTRY.register()
class StandardROIHeads_Copy2(ROIHeads):
    """
    It's "standard" in a sense that there is no ROI transform sharing
    or feature sharing between tasks.
    The cropped rois go to separate branches directly.
    ...
    def __init__(self, cfg, input_shape):
        super(StandardROIHeads_Copy2, self).__init__(cfg, input_shape)
        self._init_box_head(cfg)
    ...
    def forward(self, images, features, proposals, targets=None):
        ...
        if self.training:
            ...
            # pred_instances = self._forward_box(features_list, proposals)
            combine  = self._forward_box(features_list, proposals)

            # pdb.set_trace()

            pred_instances = self._forward_box_next(combine, proposals)
            return pred_instances, {}
    ...
    def _forward_box(self, features, proposals):
        ...
        # pdb.set_trace()
        combine = {}
        combine['cls_score'] = self.box_predictor.cls_score
        combine['box_features'] = box_features
        combine['box_pred'] = box_pred
        combine['box_prob'] = box_prob

        if self.training:
            return combine, outputs.losses()
        else:
            return combine
            # pred_instances, _ = outputs.inference(
            #     self.test_score_thresh, self.test_nms_thresh, self.test_detections_per_img
    ...
    def _forward_box_next(self, combine, proposals):
        ...
        # pdb.set_trace()
        boxes_pred = combine['box_pred']    #    B x 128 x 1024(262*4)

        boxes_prob = combine['box_prob']      # B x 128 x 263 (262 classes + background)
        boxes_feature = combine['box_features']   # B x 128 x 1024 (vector of each roi)
        cls_score = combine['cls_score']      # 263 x 1024

        # pdb.set_trace()
        batch_size = len(combine['box_pred'])
        boxes_feature = boxes_feature.reshape(batch_size, -1, boxes_feature.shape[1])

        # cat_new_f = torch.empty(batch_size, 1280 , dtype=torch.long)
        cat_new_f = []

        for i in range(batch_size):
            box_pred = boxes_pred[i]
            box_prob = boxes_prob[i]
            box_feature = boxes_feature[i]
            # pdb.set_trace()

            new_f = self.gcn(box_pred, box_prob, box_feature, cls_score)
            cat_new_f.append(new_f)
            # cat_new_f = torch.stack([i for i in cat_new_f], dim = 1)

            # pdb.set_trace()

        # cat_new_f = torch.stack([i for i in cat_new_f], dim = 0)
        cat_new_f = torch.cat([i for i in cat_new_f], dim = 0)

        if self.training:
            new_losses = self.calculate_added_loss(cat_new_f, proposals)

            return new_losses
        else:
            pred_instances = self.calculate_added_loss(cat_new_f, proposals)
            return pred_instances
    ...
    def gcn(self, boxes_pred, boxes_prob, boxes_feature, cls_score):
        # import torch.nn as nn
        # import torch.nn.functional as F
        # from fsdet.conv.gmm_conv import GMMConv
        # fc_channels = boxes_feature.shape[1]
        ...
        # num_classes = 262
        # bbox_pred_reshape = boxes_pred.view(-1, 262 , 4)

        # *********change shape corresponding to class num
        n_class = cls_w.shape[0]
        bbox_pred_reshape = boxes_pred.view(-1, n_class-1 , 4)
...
class StandardROIHeads2(ROIHeads):
    ...
    def __init__(self, cfg, input_shape):
        super(StandardROIHeads2, self).__init__(cfg, input_shape)
        self._init_box_head(cfg)
    ...
    def _forward_box(self, features, proposals):
        ...
        box_features = self.box_head(box_features)  # [None, FC_DIM]
        # pdb.set_trace()
        # print("box feature**************", box_features.shape)
        pred_class_logits, pred_proposal_deltas = self.box_predictor(box_features)
...
@ROI_HEADS_REGISTRY.register()
class StandardROIHeads_Copy3(ROIHeads):
    """
    It's "standard" in a sense that there is no ROI transform sharing
    or feature sharing between tasks.
    The cropped rois go to separate branches directly.
    ...
    def __init__(self, cfg, input_shape):
        super(StandardROIHeads_Copy3, self).__init__(cfg, input_shape)
        self._init_box_head(cfg)
    ...
    def gcn(self, boxes_pred, boxes_prob, boxes_feature, cls_score):
        ...
        # *********change shape corresponding to class num
        # n_class = cls_w.shape[0]
        # bbox_pred_reshape = boxes_pred.view(-1, n_class , 4)

        bbox_pred_reshape = boxes_pred.view(-1, 96 , 4)
        ...
        ind = torch.randint(0, 128, (4096,1)).view(-1)
        relation[1] = ind
