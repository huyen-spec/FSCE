...
__all__ = ["RCNN_SGRN", "ProposalNetwork_b"]

@META_ARCH_REGISTRY.register()
class RCNN_SGRN(nn.Module):
    """
    Try to incoporate SGRN from graph information to the meta arch of faster RCNN. Any models that contains the following three components:
    ...
    def __init__(self, cfg):
        ...
        if a ==0:
            for p in self.roi_heads.box_predictor.parameters():
                p.requires_grad = False
            print('froze roi_head box_predictor parameters')
        # pdb.set_trace()
    ...
    def forward(self, batched_inputs):
        ...
        # print("self.roi_heads.box_head.fc2*******************", self.roi_heads.box_head.fc2.weight)
        ...
        _, detector_losses, new_losses = self.roi_heads(images, features, proposals, gt_instances)

        # detector_losses = combine[4]
        ...
        #     # adjacency matrix
        #     z = relation_fc_1.cuda()(boxes_feature)
        #     z = F.relu(relation_fc_2.cuda()(z))
        ...
        #     f = gaussian(represent, relation, U)

        #     f2 = F.relu(sg_conv_1(f))
        #     h = F.relu(sg_conv_2(f2))
        ...
        # def added_loss_calculation(new_features, num_class):
        #     pass
        ...
        # new_loss = self.roi_heads.calculate_added_loss(new_f, proposals,  gt_instances)
        # _, combine = self.roi_heads(images, features, proposals, gt_instances)

        new_losses['loss_cls_new'] = new_losses['loss_cls']
        new_losses['loss_box_reg_new'] = new_losses['loss_box_reg']
        new_losses = {key: new_losses[key] for key in new_losses if key not in ['loss_cls', 'loss_box_reg']}
...
@META_ARCH_REGISTRY.register()
class ProposalNetwork_b(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.device = torch.device(cfg.MODEL.DEVICE)
